...
import com.techfly.demo.util.SizeUtil;
...
public class ADTextView extends View {
    ...
    private int mContentTextTest; //测试内容
    private List<PlatformNoticeBean.DataEntity> mTexts; //显示文字的数据源
    ...
    private void obtainStyledAttrs(AttributeSet attrs) {
        ...
        mFrontTextSize = (int) array.getDimension(R.styleable.ADTextView_ad_text_front_size, SizeUtil.Sp2Px(getContext(), 14));
        mContentTextSize = (int) array.getDimension(R.styleable.ADTextView_ad_text_content_size, SizeUtil.Sp2Px(getContext(), 14));
        mContentTextTest = (int) array.getDimension(R.styleable.ADTextView_ad_text_content_test,2);
        array.recycle();
    }
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                if (OnItemClickListener != null) {
                    OnItemClickListener.onClick(mTexts.get(mIndex).getTitle(),mTexts.get(mIndex).getContent());
                }
                break;
        }
        return true;
...
//        LogsUtil.normal("measureWidth");
    ...
    //设置数据源
    public void setmTexts(List<PlatformNoticeBean.DataEntity> mTexts) {

//        LogsUtil.normal("setmTexts");

        this.mTexts = mTexts;

        //这样就需要调用invalidate或者postInvalidate方法,主动调起onDraw方法
        postInvalidate();
...
//        LogsUtil.normal("onDraw,mTexts="+mTexts);

        if (mTexts != null) {
            PlatformNoticeBean.DataEntity model = mTexts.get(mIndex);
            String font = model.getTitle();
...
//        LogsUtil.normal("onDraw,isMove="+isMove+",isPaused="+isPaused+",hasInit="+hasInit);
